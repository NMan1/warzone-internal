...
nk_d3d11_render(ID3D11DeviceContext* context, enum nk_anti_aliasing AA)
{
	...
	{/* load draw vertices & elements directly into vertex + element buffer */
		...
		{/* fill converting configuration */
			...
			crt::memset(&config, 0, sizeof(config));
...
nk_d3d11_get_projection_matrix(int width, int height, float* result)
{
	...
	matrix[3][0] = (R + L) / (L - R);
	matrix[3][1] = (T + B) / (B - T);
	crt::memcpy(result, matrix, sizeof(matrix));
}
...
nk_d3d11_resize(ID3D11DeviceContext* context, int width, int height)
{
	...
	//ID3D11DeviceContext_Map(context, (ID3D11Resource*)d3d11.const_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped)
		...
		if (mem)
		{
			...
			if (size)
			{
				...
				if (wstr)
				{
					...
					if (utf8size)
					{
						char* utf8 = (char*)crt::malloc(utf8size);
						if (utf8)
						{
...
nk_d3d11_init(ID3D11Device* device, int width, int height, unsigned int max_vertex_buffer, unsigned int max_index_buffer)
{
	...
	/* vertex shader */
	//ID3D11Device_CreateVertexShader(device, nk_d3d11_vertex_shader, sizeof(nk_d3d11_vertex_shader), NULL, &d3d11.vertex_shader)
	...
	/* pixel shader */
	// ID3D11Device_CreatePixelShader(device, nk_d3d11_pixel_shader, sizeof(nk_d3d11_pixel_shader), NULL, &d3d11.pixel_shader);
